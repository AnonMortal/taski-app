...
export function ApplyMission() {
    ...
    api.missions
      ...
      .then((raw) => {
        ...
        setMission({
          ...
          title: raw.title ?? t('Untitled mission'),
      ...
      .catch((err: any) => {
        if (!cancelled) setError(err?.message ?? t('Mission not found'));
      })
      .finally(() => {
  ...
  const deadlineLabel = useMemo(() => {
    ...
    if (!target) return t('Open');
    const ms = new Date(target).getTime() - Date.now();
    if (ms <= 0) return t('Closed');
    const mins = Math.floor(ms / 60_000);
    if (mins < 60) return t('{{mins}} min', { mins });
    const hours = Math.floor(mins / 60);
    if (hours < 24) return t('{{hours}}h {{mins}}m', { hours, mins: mins % 60 });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setIsSubmitted(true);
      showSuccess(t('Application submitted'));
      setTimeout(() => {
        navigate('/marketplace');
      }, 1800);
    } catch (err: any) {
      const msg = err?.message ?? t('Failed to apply');
      showError(msg);
    } finally {
      setSubmitting(false);
    }
  ...
  const getMatchBadge = (score: number) => {
    if (score >= 75) return { text: t('Excellent Match'), color: 'bg-green-500' };
    if (score >= 50) return { text: t('Good Match'), color: 'bg-[#4B3EEF]' };
    return { text: t('Low Match'), color: 'bg-red-500' };
  };
    ...
    return (
      <main className="flex-1 flex items-center justify-center p-8">
        <div className="text-center">
          <p className="text-sm text-red-600 mb-3">{error ?? t('Mission not found')}</p>
    ...
    return (
      <main className="flex-1 overflow-y-auto p-4 md:p-8 flex items-center justify-center">
        ...
        >
          ...
          >
            {t("You'll be assigned the mission once the acceptance window closes.")}
          </motion.p>
        </motion.div>
      </main>
    );
    ...
    <main className="flex-1 overflow-y-auto p-4 md:p-8">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h2 className="text-2xl md:text-3xl font-bold text-[#1A1B25]">{t('Apply to Mission')}</h2>
            <p className="text-sm text-gray-600">{t('Select your agent and submit your application')}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          >
            <h3 className="text-xl font-bold text-[#1A1B25] mb-4">{t('Mission Overview')}</h3>
            ...
            <div className="space-y-4">
                ...
                <p className="text-sm text-gray-600">
                  ...
                  <span className="font-semibold text-gray-900">
                    {mission.posterType === 'ENTERPRISE'
                      ? mission.companyName ?? t('Enterprise')
                      : t('Individual')}
                  </span>
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#4B3EEF]/10 border border-[#4B3EEF]/30">
                  ...
                  <div>
                    <p className="text-xs text-gray-600">{t('Time Left')}</p>
          ...
          >
            ...
            {userAgents.length === 0 ? (
              <p className="text-sm text-gray-500 py-6 text-center">
                {t('No agents available. The agent runner must be online and you must hold an Agent Passport.')}
              </p>
            ) : (
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className={`px-3 py-1 rounded-lg border ${getMatchColor(agent.matchScore)}`}>
                          <p className="text-xs font-bold">{t('{{score}}% Match', { score: agent.matchScore })}</p>
                        </div>
                      </div>

                      <div className="grid grid-cols-3 gap-3 text-center">
                        <div>
                          <p className="text-xs text-gray-600">{t('Reputation')}</p>
                        ...
                        <div>
                          <p className="text-xs text-gray-600">{t('Staked')}</p>
                      ...
                      {agent.matchScore >= 75 && (
                        ...
                        >
                          ...
                          <p className="text-xs font-semibold">{t('{{badge}} — Recommended', { badge: matchBadge.text })}</p>
                        </div>
                      )}
                    </motion.button>
                  );
        ...
        <div className="space-y-6">
          ...
          >
            <h3 className="text-lg font-bold text-[#1A1B25] mb-4">{t('Application Summary')}</h3>

            {selectedAgent ? (
              <div className="space-y-4">
                <div className="p-4 rounded-xl bg-indigo-50 border border-indigo-200">
                  <p className="text-xs text-gray-600 mb-1">{t('Selected Agent')}</p>
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-700 mb-3">{t('Bounty Distribution')}</p>
                  <div className="space-y-2">
                    <div className="flex justify-between items-center p-2 rounded-lg bg-green-50">
                      <span className="text-xs text-gray-600">{t('Winning Agent (70%)')}</span>
                    ...
                    <div className="flex justify-between items-center p-2 rounded-lg bg-indigo-50">
                      <span className="text-xs text-gray-600">{t('Treasury — Buyback (20%)')}</span>
                    ...
                    <div className="flex justify-between items-center p-2 rounded-lg bg-purple-50">
                      <span className="text-xs text-gray-600">{t('Reward Pool (10%)')}</span>
                      <span className="text-sm font-bold text-purple-700">${rewardPoolShare.toFixed(2)}</span>
                    </div>
                  </div>
                  <p className="text-[10px] text-gray-500 mt-2">
                    {t('On-chain PaymentSplitter pays 70% to the winning agent and 30% to the treasury. The treasury routes 20% to $TASK buyback and 10% to the reward pool off-chain.')}
                  </p>
                </div>

                <div className="pt-4 border-t border-gray-200">
                  <p className="text-xs text-gray-600 mb-2">{t('Agent Stake')}</p>
                ...
                >
                  ...
                  <span className="relative z-10">
                    {acceptanceClosed ? t('Acceptance Window Closed') : submitting ? t('Submitting…') : t('Submit Application')}
                  </span>
                </motion.button>
              </div>
            ) : (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-500">{t('Select an agent to continue')}</p>
              </div>
            )}
          </motion.div>
        </div>
